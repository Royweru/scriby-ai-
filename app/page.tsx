...
          disabled={!canRecord || !!audioUrl || isUploading} // Disable recording if a file is selected or uploading
          className={`
...
          disabled={isRecording || !!audioUrl || isUploading} // Disable upload if recording, file present, or uploading
          className={`
            w-24 h-24 rounded-full flex flex-col items-center justify-center 
            transition-all duration-300 transform shadow-2xl text-xs
            ${audioUrl 
...
        accept="audio/*"
        style={{ display: 'none' }}
        disabled={isRecording || !!audioUrl || isUploading} // Also disable file input during upload
      />

      <div className="flex flex-col space-y-4">
        {/* Main Action Button --- MODIFIED BLOCK --- */}
        <button
          onClick={onUpload}
          // MODIFICATION 1: Add isUploading to the disabled check
...
        >
          {/* MODIFICATION 2: Show loading state when isUploading is true */}
          {isUploading ? (
            <>
              {/* SVG Spinner */}
...
        {audioUrl && !isUploading && ( // Hide the audio player during upload
...
      // The finally block will still run, re-enabling the UI
    } finally {
      // Clean up local resources
      setIsUploading(false);
      setAudioUrl(null); // Clear the audio file from the UI
      audioBlobRef.current = null;
    }
  }, []);
...
s             <pre className="whitespace-pre-wrap font-mono text-slate-300">
                    {transcript}
                </pre>
in           </div>
